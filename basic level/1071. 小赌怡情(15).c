...
using namespace std;
int main() {
	int total_times, money;
	cin >> money >> total_times;

	for (int i = 0; i < total_times; i++) {
		int first, guess, num_of_gamble, second;
		cin >> first >> guess >> num_of_gamble >> second;
		if (num_of_gamble > money) {
			cout << "Not enough tokens.  Total = " << money << "." << endl;
			continue;
		}
		if ((second > first?1:0) == guess) {
			//printf("Win %d!  Total = %d.\n", num_of_gamble, money + num_of_gamble);
			money += num_of_gamble;
			cout << "Win " << num_of_gamble << "!  Total = " << money << "." << endl;

		}
		else {
			money -= num_of_gamble;
			cout << "Lose " << num_of_gamble << ".  Total = " << money << "." << endl;
